...
TRADER.prototype.analyzeMarket = function (next) {
	...
	async.eachSeries(work_balances, function (balance, serie_callback) {
		...
		fnStack.push(self.baseConnector.updateOrder.bind(self, order.exchangeId, { 
...
TRADER.prototype.getUserOrders = function (next) {
	...
	this.getOrders({}, function (API_ORDERS) {
		...
		self.baseConnector.findOrders({}, (err, ORDERS_FROM_BASE) => {
			...
			ORDERS = ORDERS.filter(el => moment(el.lastModificationTime).isSameOrAfter(moment().subtract(7, 'd'), 'd') || el.orderStatus == 'OPEN');
